...
import gov.pbc.xjcloud.common.security.util.ConcurrentDateFormat;

public class XjcloudFeignErrorDecoder extends ErrorDecoder.Default {
	...
	public Exception decode(String methodKey, Response response) {
		XjcloudFeignException exception = errorStatus(methodKey, response);
	...
	private static XjcloudFeignException errorStatus(String methodKey, Response response) {
		try {
			if (response.body() != null) {
				Reader reader = response.body().asReader();
				return new XjcloudFeignException(R.builder()
		...
		return new XjcloudFeignException(message);
	}
